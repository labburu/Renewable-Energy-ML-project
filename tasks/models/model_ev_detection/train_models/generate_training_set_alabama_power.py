...
df_accounts = spark.read.parquet('s3://tendril-airflow-prod/dags/soa_daily/'
                                 'transform_accounts/2019-10-18/soa_daily__transform_accounts__20191018.parquet')
...
# df_accounts = df_accounts.select('id')
...
df_accounts = df_accounts.where(col('ACCT_NUM').isNull()).limit(9000)
df_accounts = df_accounts.drop('ACCT_NUM')

df_locations = spark.read.parquet('s3://tendril-airflow-prod/dags/soa_daily/'
                                  'transform_locations/2019-09-30/soa_daily__transform_locations__20190930.parquet')
...
df_channels = spark.read.parquet('s3://tendril-airflow-prod/dags/soa_daily/'
                                 'transform_channels/2019-09-30/soa_daily__transform_channels__20190930.parquet')
