"""Ingest and map all ALLIANT location/account/meter identifiers to Tendril channel_id"""
...
class ChannelIngestAlliant(ChannelIngest):

    @staticmethod
    def load_ami(spark, table_name='ami'):
        """Load DataFrame with data from ALLIANT AMI CSVs.
        ...
        return spark \
            ...
            .withColumnRenamed('premise_id', 'external_location_id') \
            .withColumnRenamed('meter_id', 'external_channel_id') \
            .withColumnRenamed('meter_type', 'direction') \
            .withColumn(
                'external_account_id',
    ...
    def run(self, spark):
        """Main Entry Point for ALLIANT Channel ingest."""
        df_ami_ids = ChannelIngestAlliant \
            .load_ami(spark)
