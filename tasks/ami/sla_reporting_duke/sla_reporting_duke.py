...
class AmiSlaReportingDuke:
    def __init__(self, **kwargs):
        ...
        self.jurisdiction_interval_detail_path = kwargs.get('jurisdiction_interval_detail_path')
    ...
    def save_output(self, df, output_type):
        ...
        if output_type == 1:  # Daily QC report
            ...
            save_format = 'csv'
        elif output_type == 6:  # Jurisdiction Interval
            save_to = self.jurisdiction_interval_detail_path
            save_format = 'csv'
        else:
            save_to = None
        ...
        if save_to is None:
            ...
            else:  # Manifest | Jurisdiction-interval detail
                df.coalesce(1) \
                    .write.save(
                    path=save_to,
                    mode='overwrite',
    ...
    def get_and_save_manifest(self, spark):
        """"Get manifest file from Duke SFTP and save it to meter-data bucket for long term storage."""
        try:
    ...
    def get_and_save_jurisdiction_interval_detail(self, spark):
        """"Get jurisdiction-interval detail from manifest history and save it for long term storage."""
        try:
            df_historical = spark.read.csv(self.manifest_historical_path, header=True) \
                .select(
                    substring_index(substring_index(col('file_name'), '_', 3), '_', -2).alias('jurisdiction_interval')
                ) \
                .distinct() \
                .orderBy(col('jurisdiction_interval'))

            df_execution_date = spark.read.csv(self.manifest_save_path, header=True)  \
                .select(
                    substring_index(substring_index(col('file_name'), '_', 3), '_', -2).alias('jurisdiction_interval'),
                    col('line_count')
                ) \
                .groupBy(
                    col('jurisdiction_interval')
                ) \
                .agg(
                    (f.count('*')).cast('int').alias('total_file_count'),
                    (f.sum(col('line_count'))).cast('int').alias('total_line_count')
                ) \
                .orderBy(col('jurisdiction_interval'))
            ...
            df = df_historical \
                .join(
                    df_execution_date,
                    ['jurisdiction_interval'],
                    'left_outer'
                ) \
                .na.fill(0) \
                .orderBy(
                    col('jurisdiction_interval').asc()
                )

            self.save_output(df, 6)
            return
    ...
    def create_and_save_daily_sla_report(self, spark, df):
        """"This will consume the daily QC report and will create a daily SLA report for Duke."""
    ...
    def create_and_save_historical_sla_report(self, spark, df):
        ...
        try:
            ...
            return
        # Add order by column
        df_historical = AmiSlaReportingDuke.add_sla_orderby(spark, df_historical)
    ...
    def create_qc_fail_payload(self, park, df, payload_type=None):
        ...
        if payload_type == 'sla':
            sla_flag = 1
            title = "*Duke AMI Ingest QC Errors for SLA Metrics for {}*".format(self.execution_date)
        else:
            sla_flag = 0
            title = "*Duke AMI Ingest QC Errors for Non-SLA Metrics for {}*".format(self.execution_date)
        ...
        headers = AmiSlaReportingDuke.create_slack_payload_string_from_list(header_list)
        values = AmiSlaReportingDuke.create_slack_payload_string_from_list(value_list)
        output = []
        output.append(title)
    ...
    def create_sla_value_string_for_slack_callout(spark, df):
        ...
        try:
            ...
            output = AmiSlaReportingDuke.create_slack_payload_string_from_list(value_list)
    ...
    def create_daily_sla_input(self, spark, df):
        ...
        title = "*Duke AMI Ingest SLA Report for {}*".format(self.execution_date)
        metric_name_list = \
            [
                'File Count',
                'Row Count', 'Row Ingested', 'Rows Uningested', 'Raw to Hourly', 'Raw to Daily', 'Ingest Time'
            ]
        headers = AmiSlaReportingDuke.create_slack_payload_string_from_list(metric_name_list)
        values = AmiSlaReportingDuke.create_sla_value_string_for_slack_callout(spark, df)
        output = []
        output.append(title)
        output.append(headers)
        output.append(values)
    ...
    def create_slack_table_payload(self, input_list):
        ...
        if 'Duke AMI Ingest QC Errors for SLA Metrics' in input_list[0]:
            alert = 'danger'
            right_column_title = 'QC Details'
        elif 'Duke AMI Ingest QC Errors for Non-SLA Metrics' in input_list[0]:
            alert = 'warning'
            right_column_title = 'QC Details'
        else:
            alert = 'good'
        ...
        if 'Errors' not in input_list[0]:
            link = 'https://console.aws.amazon.com/s3/buckets/tendril-meter-data-prod/quality/{}/{}/sla_report/sla_report.csv/?region=us-east-1&tab=overview'.format(self.tenant_id, self.execution_date)  # noqa
        else:
            link = 'https://console.aws.amazon.com/s3/buckets/tendril-meter-data-prod/quality/{}/{}/qc_report/qc_report.parquet/?region=us-east-1&tab=overview'.format(self.tenant_id, self.execution_date)  # noqa
    ...
    def create_daily_file_count_slack_payload(self, spark, df):
        ...
        title = '*Duke AMI Ingest Daily File Count Sanity Check for {}*'.format(self.execution_date)
        path_blurb = 'https://console.aws.amazon.com/s3/buckets/tendril-meter-data-prod/quality/11/{}/manifest/manifest.csv/?region=us-east-1&tab=overview'.format(self.execution_date)  # noqa
        ...
        if int(delivered_file_count) >= self.daily_file_quota:
            alert = 'good'
            file_count_message = \
                'Manifest Files: {} >= Duke Daily Quota {}.'.format(
    ...
    def create_daily_read_count_payload(self, spark, df):
        ...
        title = '*Duke AMI Ingest Daily Read Count Sanity Check for {}*'.format(self.execution_date)
        link = 'https://console.aws.amazon.com/s3/buckets/tendril-meter-data-prod/quality/{}/{}/qc_report/qc_report.parquet/?region=us-east-1&tab=overview'.format(self.tenant_id, self.execution_date)  # noqa
    ...
    def run(self, spark):
        ...
        # Get and save jurisdiction-interval detail
        log.info('get_and_save_jurisdiction_interval_detail')
        self.get_and_save_jurisdiction_interval_detail(spark)
